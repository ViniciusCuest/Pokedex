...
export function SelectOption({ data }: { data: GenerationProps[]; }) {
  ...
  return (
    <div className='relative sm:mr-4 bg-max_red w-36 sm:w-64 rounded-lg cursor-pointer select-none'>
      <div
        className='p-2 flex flex-row items-center justify-between'
        onClick={() => {
          setOpen(prev => !prev);
        }}>
        <label className='cursor-pointer font-sans text-sm sm:text-lg font-semibold text-white'>
          {
      ...
      <motion.ul
        ...
        className={`${open ? 'flex' : 'hidden'} mt-2 rounded-lg flex-col flex-grow-0 absolute z-50 sm:mr-5 bg-white w-full shadow-lg`}>
        {
          data.map((item) => (
            <li
              key={item.id}
              value={item.id}
              className='flex flex-grow flex-row text-center p-2 items-center justify-around font-sans text-sm sm:text-lg border-t-[1px] gap-x-2 border-background'
              onClick={(evt) => {
                evt.preventDefault();
            ...
            >
              ...
              <span className='flex flex-row items-center gap-x-2'>
                <GrMapLocation className='hidden sm:flex w-5 h-5' color={`${fullConfig?.theme?.colors.black_900}`}  />
                {
